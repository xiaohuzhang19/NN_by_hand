...
class SoftmaxRegression(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        #############################################################################
        Z=np.dot(X,self.weights['W1'])  #shape N,10
        A=self.ReLU(Z) #shape N,10
        p=self.softmax(A)
        ...
        #############################################################################
        #reference: https://www.youtube.com/watch?v=aeM-fmcdkXU 
        #creating one-hot
        if mode == 'train': 
            #creating one-hot
            ...
            dlda=p-y_onehot
            gradient = dlda
            dadz=self.ReLU_dev(Z)
            dzdw=X.T
            dldz=gradient*dadz
            dldw=np.dot(dzdw,dldz)/X.shape[0]
